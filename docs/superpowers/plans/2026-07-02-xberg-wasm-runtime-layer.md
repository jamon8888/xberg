...
- **Testing:** `vitest` with 80%+ coverage target; no network calls in CI (use fixture models / mocked Promises / stub ORT sessions).
...
describe("embedder", () => {
  ...
  beforeAll(async () => {
    ...
    embedder = await createEmbedder({
      models: { embedder: "Xenova/minilm-l6-v2" },
    });
  });
...
const DEFAULT_MODEL = "Xenova/minilm-l6-v2";
...
describe("NER", () => {
  ...
  beforeAll(async () => {
    ner = await createNer({
      models: { ner: "Xenova/gliner2-small-onnx" },
    });
  });
...
const DEFAULT_NER_MODEL = "Xenova/gliner2-small-onnx";
...
**Parameters:**
- `config.models?.embedder` — transformers.js model ID (default: "Xenova/minilm-l6-v2")
