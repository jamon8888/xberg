...
public struct CustomGlinerSource: Codable, Sendable, Hashable {
    ...
    public let architecture: GlinerArchitecture
    public init(repo: String, modelFile: String, tokenizerFile: String, architecture: GlinerArchitecture) {
        self.repo = repo
...
// MARK: - Internal FFI conversions for CustomGlinerSource
internal extension CustomGlinerSource {
    init(_ rb: RustBridge.CustomGlinerSourceRef) throws {
        self.repo = rb.repo().toString()
        self.modelFile = rb.modelFile().toString()
        self.tokenizerFile = rb.tokenizerFile().toString()
        self.architecture = try { let rawValue = rb.architecture().toString(); guard let value = GlinerArchitecture(rawValue: rawValue) else { throw RasterizeError.validation(message: "Unknown GlinerArchitecture variant", source: rawValue) }; return value }()
    }

    func intoRust() throws -> RustBridge.CustomGlinerSource {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.customGlinerSourceFromJson(json)
    }
}
...
public typealias CustomGlinerSourceRef = RustBridge.CustomGlinerSourceRef

public typealias CustomGlinerSourceRefMut = RustBridge.CustomGlinerSourceRefMut
...
public func pageImageFromJson(_ json: String) throws -> PageImage {
    return try RustBridge.pageImageFromJson(json)
}
...
public func customGlinerSourceFromJson(_ json: String) throws -> CustomGlinerSource {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CustomGlinerSource.self, from: data)
}
...
public func customSourceFromParts(repo: String? = nil, modelFile: String? = nil, tokenizerFile: String? = nil, architecture: GlinerArchitecture? = nil) throws -> CustomGlinerSource? {
    ...
    return try RustBridge.customSourceFromParts(_rb_repo, _rb_modelFile, _rb_tokenizerFile, _rb_architecture).map { try CustomGlinerSource($0) }
}
