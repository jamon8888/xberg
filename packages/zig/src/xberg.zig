...
pub fn extract(input: []const u8, config: []const u8) XbergError![]u8 {
    const input_z = try std.fmt.allocPrintSentinel(std.heap.c_allocator, "{s}", .{input}, 0);
...
pub fn extract_batch(inputs: []const u8, config: []const u8) XbergError![]u8 {
    ...
    const inputs_z = try std.fmt.allocPrintSentinel(std.heap.c_allocator, "{s}", .{inputs}, 0);
...
pub fn verify_excerpt(excerpt: []const u8, source_text: []const u8) error{OutOfMemory}!bool {
    const excerpt_z = try std.fmt.allocPrintSentinel(std.heap.c_allocator, "{s}", .{excerpt}, 0);
    defer std.heap.c_allocator.free(excerpt_z);
    const source_text_z = try std.fmt.allocPrintSentinel(std.heap.c_allocator, "{s}", .{source_text}, 0);
