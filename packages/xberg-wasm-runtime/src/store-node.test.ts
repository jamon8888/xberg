...
describe("node vector store (better-sqlite3 + sqlite-vec)", () => {
  ...
  it("upserts a document with chunks and queries by real vec0 similarity", async () => {
    ...
    const chunks: ChunkRecord[] = [
      { sourceId: "src-1", chunkIndex: 0, text: "apple fruit", startOffset: 0, endOffset: 11, embedding: new Float32Array([1, 0, 0, 0]) },
      { sourceId: "src-1", chunkIndex: 1, text: "apple tree", startOffset: 12, endOffset: 22, embedding: new Float32Array([0, 1, 0, 0]) },
    ];
  ...
  it("deletes a document and its chunks are no longer queryable", async () => {
    await store.ensureCollection(testCollection, vectorDim);
    const doc: DocumentRecord = { documentId: "doc-1", sourceId: "src-1", collectionId: testCollection };
    const chunk: ChunkRecord = { sourceId: "src-1", chunkIndex: 0, text: "hello", startOffset: 0, endOffset: 5, embedding: new Float32Array([1, 0, 0, 0]) };
