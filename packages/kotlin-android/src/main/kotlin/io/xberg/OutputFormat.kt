 ...
 *
 * Controls the format of the `content` field in `ExtractedDocument`. When set to `Markdown`,
 * `Djot`, or `Html`, the output uses that format. `Plain` returns the raw extracted text.
 * `Structured` returns JSON with full OCR element data including bounding boxes and confidence
 * scores.
 */
...
private class OutputFormatDeserializer :
    com.fasterxml.jackson.databind.deser.std.StdDeserializer<OutputFormat>(
        OutputFormat::class.java
    ) {
    ...
    override fun deserialize(
            ...
            return when (node.asText()) {
                ...
                "structured" -> OutputFormat.Structured
                    ...
                    throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
                        parser,
                        "Unknown OutputFormat unit variant",
                        node.asText(),
                        OutputFormat::class.java,
                    )
            }
                ...
                if (!it.hasNext()) {
                    ...
                    return when (entry.key) {
                        "custom" ->
                            ...
                            throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
                                parser,
                                "Unknown OutputFormat data variant",
                                entry.key,
                                OutputFormat::class.java,
                            )
                    }
        ...
        throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
            parser,
            "Cannot deserialize OutputFormat: expected string or single-field object",
            null,
            OutputFormat::class.java,
        )
    }
