...
sealed class PiiCategory {
     ...
     *
     * Surfaced by the redaction engine when a hit comes from `RedactionConfig.custom_terms` or
     * `RedactionConfig.custom_patterns`. The string is the label passed alongside the term/pattern.
     * Use those fields rather than constructing `Custom` directly via the `categories` filter — the
     * pattern engine cannot detect arbitrary text from a category name alone.
     */
    data class Custom(val value: String) : PiiCategory()
}

private class PiiCategoryDeserializer :
    com.fasterxml.jackson.databind.deser.std.StdDeserializer<PiiCategory>(PiiCategory::class.java) {
    ...
    override fun deserialize(
            ...
            return when (node.asText()) {
                ...
                "location" -> PiiCategory.Location
                    ...
                    throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
                        parser,
                        "Unknown PiiCategory unit variant",
                        node.asText(),
                        PiiCategory::class.java,
                    )
            }
        }
        if (node.isObject) {
                ...
                if (!it.hasNext()) {
                    ...
                    return when (entry.key) {
                        "custom" ->
                            ...
                            throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
                                parser,
                                "Unknown PiiCategory data variant",
                                entry.key,
                                PiiCategory::class.java,
                            )
                    }
        ...
        throw com.fasterxml.jackson.databind.exc.InvalidFormatException(
            parser,
            "Cannot deserialize PiiCategory: expected string or single-field object",
            null,
            PiiCategory::class.java,
        )
    }
...
private class PiiCategorySerializer :
    com.fasterxml.jackson.databind.ser.std.StdSerializer<PiiCategory>(PiiCategory::class.java) {
    @Suppress("LongMethod")
