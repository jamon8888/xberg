...
enum class RedactionStrategy {
    ...
    /**
     * Replace with a SHA-256 hash of the original value (truncated to 16 hex chars). Lets
     * downstream consumers do equality joins without recovering the source.
     */
    @com.fasterxml.jackson.annotation.JsonProperty("hash") HASH,
    /**
     * Replace with a per-category running token (`"[PERSON_1]"`, `"[PERSON_2]"`, …) so the same
     * person referenced twice gets the same token within the document.
     */
