...
object Xberg {
    ...
    fun extractBatch(inputs: List<ExtractInput>, config: ExtractionConfig): ExtractionResult {
        val resultJson =
            XbergBridge.nativeExtractBatch(
     ...
     *
     * Returns every file extension Xberg recognizes together with its corresponding MIME type,
     * derived from the central format registry. Formats that have no registered file extension
     * (such as source code, which is detected dynamically) are not included.
     *
     ...
     *
     * Returns lines that assert a claim but carry neither a footnote citation anchor (`[^...]`) nor
     * an inference marker (`[*inference*]`).
     *
     * The heuristic is simple: a line that contains alphabetic words, ends with sentence
     * punctuation, and is not a heading, blank line, or markup-only line is considered a claim.
     ...
     *
     * Performs exact matching by default. Also tries whitespace-normalized matching (collapsing
     * runs of whitespace on both sides) since PDF-extracted text often has irregular spacing.
     *
     * **Returns:**
     *
    ...
    fun verifyExcerpt(excerpt: String, sourceText: String): Boolean =
        XbergBridge.nativeVerifyExcerpt(excerpt, sourceText)
}
