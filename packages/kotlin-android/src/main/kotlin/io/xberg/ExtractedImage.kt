...
data class ExtractedImage(
     ...
     * (`crates/xberg/src/plugins/processor/builtin/captioning.rs`), which routes each image through
     * `crate.llm.region_extractor.extract_region_with_vlm` in caption mode. `null` when captioning
     * is disabled or the VLM declined to caption.
     */
    val caption: String? = null,
    /**
     * QR codes decoded from this image, when QR detection is enabled.
     *
     * Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust
     * `rqrr` decoder. `null` when QR detection is disabled; an empty `Some([])` when detection ran
     * but found nothing.
     */
    val qrCodes: List<QrCode>? = null,
    /**
     * Base64-encoded copy of `data`; populated when `ImageExtractionConfig.include_data_base64` is
     * `true`. Omitted from JSON by default; use instead of `data` in JSON-only clients.
     */
