...
data class PdfConfig(
     ...
     *
     * When `true` (default), reads the document's interactive form structure (field names, types,
     * values, widget geometry). Cheap and strictly additive — non-form PDFs simply yield an empty
     * list. Set to `false` to skip the form pass entirely.
     */
    val extractFormFields: Boolean = true,
    /**
     * Reorder extracted text by layout-detected reading order.
     *
     * When `true`, projects text spans onto layout-detected regions, performs column detection, and
     * emits spans in natural reading order (important for multi-column academic PDFs). Requires the
     * `layout-detection` feature; has no effect without it. Defaults to `false`.
     */
