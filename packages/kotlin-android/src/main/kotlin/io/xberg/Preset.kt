// Generated by alef. Do not edit by hand.
// alef:hash:350ae5725f1360a336e35cba3e068d84a71c9fa0d1644e646b1021e0ad1dc3f1
@file:Suppress(
    "ktlint:standard:trailing-comma-on-call-site",
    "ktlint:standard:trailing-comma-on-declaration-site",
    "ktlint:standard:spacing-between-declarations-with-comments",
    "ktlint:standard:spacing-between-declarations-with-annotations",
    "ktlint:standard:when-entry-bracing",
    "ktlint:standard:blank-line-between-when-conditions",
    "ktlint:standard:blank-line-before-declaration",
    "ktlint:standard:chain-method-continuation",
    "ktlint:standard:annotation",
    "ktlint:standard:max-line-length",
    "ktlint:standard:no-semi",
    "ktlint:standard:statement-wrapping",
    "MaxLineLength",
    "TooManyFunctions",
    "FunctionParameterNaming",
    "LongParameterList",
    "CyclomaticComplexMethod",
    "LongMethod",
    "MagicNumber",
    "NestedBlockDepth",
    "ReturnCount",
    "UnusedParameter",
)

package io.xberg

/**
 * A curated structured-extraction preset loaded from the embedded library.
 *
 * Each preset is a JSON file under `src/presets/library/<id>/v1.json` that validates against the
 * meta-schema in `src/presets/preset.schema.json`.
 *
 * Downstream catalog consumers can inject presets via `extend_from_dir`. The embedded OSS library
 * ships only the `generic_document` toy preset.
 */
data class Preset(
    /** Stable, URL-safe preset identifier (lowercase snake_case). */
    val id: String,
    /** Monotonic version string (e.g. `v1`). */
    val version: String,
    /** Human-readable schema name forwarded to the LLM as the response/tool name. */
    val schemaName: String,
    /** One-line preset description shown in the registry UI. */
    val description: String,
    /** Top-level category for grouping in the playground. */
    val category: PresetCategory,
    /** Free-form tags used for search/filtering. May be empty. */
    val tags: List<String> = emptyList(),
    /** JSON Schema (Draft 2020-12) describing the structured output shape. */
    val schema: Any,
    /** Instruction primer sent to the model. */
    val systemPrompt: String,
    /** Optional mustache-style template merged with caller-supplied context. */
    val contextTemplate: String? = null,
    /** Strategy for merging per-batch outputs across paginated calls. */
    val mergeMode: MergeMode,
    /** Default call mode suggested for this preset; heuristics may override. */
    val preferredCallMode: CallMode,
    /**
     * When true, the prompt asks the model to wrap each field as `{value, page, bbox, confidence}`
     * for downstream citation overlays.
     */
    val emitCitations: Boolean,
    /** Optional bundled sample (input file + reference output) for preview. */
    val sample: PresetSample? = null,
    /**
     * Stable sha256 fingerprint of the canonical preset file contents.
     *
     * Populated at registry load — not present in the on-disk JSON files. Used as a
     * cache-invalidation token by the worker pipeline.
     */
    val fingerprint: String,
)
