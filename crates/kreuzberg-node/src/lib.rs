...
pub fn gline_backend_new(repo_id: Option<String>) -> napi::Result<JsGlineBackend> {
...
impl JsTokenCounter {
    ...
    pub fn next_token(&self, category: JsPiiCategory, original: String) -> String {
        let _ = (category, original);
        compile_error!(
            "alef cannot auto-delegate `TokenCounter.next_token`; configure an adapter body or add this item to the backend exclude list"
        )
    }
...
impl From<JsGlineBackend> for kreuzberg::GlineBackend {
    fn from(val: JsGlineBackend) -> Self {
        let _ = val;
        compile_error!(
            "alef cannot generate From<JsGlineBackend> for kreuzberg::GlineBackend because construction requires private or borrowed fields; expose a complete constructor, configure the item as opaque, or exclude it from this backend"
        )
    }
}
...
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<kreuzberg::chunking::semantic::merge::Segment<'_>> for JsSegment {
    fn from(val: kreuzberg::chunking::semantic::merge::Segment<'_>) -> Self {
        Self {
            text: val.text.to_string(),
            byte_start: val.byte_start as i64,
        }
...
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<JsSegment> for kreuzberg::chunking::semantic::merge::Segment<'_> {
    fn from(val: JsSegment) -> Self {
        let _ = val;
        compile_error!(
            "alef cannot generate From<JsSegment> for kreuzberg::chunking::semantic::merge::Segment because construction requires private or borrowed fields; expose a complete constructor, configure the item as opaque, or exclude it from this backend"
        )
    }
}
