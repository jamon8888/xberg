...
impl From<GlineBackend> for kreuzberg::GlineBackend {
    fn from(val: GlineBackend) -> Self {
        let _ = val;
        compile_error!(
            "alef cannot generate From<GlineBackend> for kreuzberg::GlineBackend because construction requires private or borrowed fields; expose a complete constructor, configure the item as opaque, or exclude it from this backend"
        )
    }
}
...
#[allow(clippy::redundant_closure, clippy::useless_conversion)]
impl From<Segment> for kreuzberg::chunking::semantic::merge::Segment<'_> {
    fn from(val: Segment) -> Self {
        let _ = val;
        compile_error!(
            "alef cannot generate From<Segment> for kreuzberg::chunking::semantic::merge::Segment because construction requires private or borrowed fields; expose a complete constructor, configure the item as opaque, or exclude it from this backend"
        )
    }
}
