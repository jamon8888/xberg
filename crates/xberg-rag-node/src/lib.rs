...
impl RagStore {
    ...
    pub async fn ensure_collection(&self, spec_json: String) -> napi::Result<()> {
        let spec: xberg_rag::CollectionSpec =
            serde_json::from_str(&spec_json).map_err(|e| napi::Error::from_reason(format!("Invalid spec: {e}")))?;
        self.inner
    ...
    pub async fn upsert_document(
        ...
        let chunks: Vec<xberg_rag::ChunkRecord> =
            serde_json::from_str(&chunks_json).map_err(|e| napi::Error::from_reason(format!("Invalid chunks: {e}")))?;
        let doc_id = self
            .inner
    ...
    pub async fn retrieve(&self, collection: String, query_json: String) -> napi::Result<String> {
        let query: xberg_rag::RetrieveQuery =
            serde_json::from_str(&query_json).map_err(|e| napi::Error::from_reason(format!("Invalid query: {e}")))?;
        let output = self
            .inner
            .retrieve(&collection, &query)
            .await
            .map_err(|e| napi::Error::from_reason(format!("retrieve failed: {e}")))?;
        serde_json::to_string(&output).map_err(|e| napi::Error::from_reason(format!("Serialization failed: {e}")))
    }

    #[napi]
    pub async fn delete_documents(&self, collection: String, ids_json: String) -> napi::Result<f64> {
        let ids: Vec<String> =
            serde_json::from_str(&ids_json).map_err(|e| napi::Error::from_reason(format!("Invalid ids: {e}")))?;
    ...
    pub async fn delete_by_filter(&self, collection: String, filter_json: String) -> napi::Result<f64> {
        let filter: xberg_rag::Filter =
            serde_json::from_str(&filter_json).map_err(|e| napi::Error::from_reason(format!("Invalid filter: {e}")))?;
        self.inner
    ...
    pub async fn collection_stats(&self, collection: String) -> napi::Result<String> {
        ...
        serde_json::to_string(&stats).map_err(|e| napi::Error::from_reason(format!("Serialization failed: {e}")))
    }
}
...
pub async fn embed_texts(texts_json: String, config_json: String) -> napi::Result<String> {
    let texts: Vec<String> =
        serde_json::from_str(&texts_json).map_err(|e| napi::Error::from_reason(format!("Invalid texts: {e}")))?;
    let config: xberg::core::config::EmbeddingConfig =
        serde_json::from_str(&config_json).map_err(|e| napi::Error::from_reason(format!("Invalid config: {e}")))?;

    let embeddings = xberg::embed_texts_async(texts, &config)
        .await
        .map_err(|e| napi::Error::from_reason(format!("Embedding failed: {e}")))?;

    serde_json::to_string(&embeddings).map_err(|e| napi::Error::from_reason(format!("Serialization failed: {e}")))
}

#[napi]
pub async fn rerank(query: String, documents_json: String, config_json: String) -> napi::Result<String> {
    let documents: Vec<String> = serde_json::from_str(&documents_json)
        .map_err(|e| napi::Error::from_reason(format!("Invalid documents: {e}")))?;
    let config: xberg::core::config::RerankerConfig =
        serde_json::from_str(&config_json).map_err(|e| napi::Error::from_reason(format!("Invalid config: {e}")))?;
    ...
    serde_json::to_string(&ranked).map_err(|e| napi::Error::from_reason(format!("Serialization failed: {e}")))
}
