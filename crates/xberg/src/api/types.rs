...
pub struct ApiState {
    ...
    ///
    /// Fork-only: this field exists only for the `/v1/process` + rehydrate
    /// API surface fenced in `api::rag`.
...
// Fork-only: the `/v1/process` + rehydrate request/response types
// (ProcessRequest, ProcessOperations, ProcessRedactOperation, ProcessResponse,
// RehydrateRequest, RehydrateResponse) live in `api::rag::types`, fenced behind
// the `process-api` feature.
